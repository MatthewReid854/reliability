...
def test_Weibull_Distribution():
    ...
    assert_allclose(dist.mean, 14.4311346272637895, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 2.316256875880522, rtol=tolerance)
    assert_allclose(dist.variance, 5.365045915063796, rtol=tolerance)
    assert_allclose(dist.skewness, 0.6311106578189344, rtol=tolerance)
    assert_allclose(dist.kurtosis, 3.2450893006876456, rtol=tolerance)
    assert dist.param_title_long == 'Weibull Distribution (α=5,β=2,γ=10)'
    assert_allclose(dist.quantile(0.2), 12.361903635387193, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 12.9861134604144417, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(20), 1.1316926249544481, rtol=tolerance)
...
def test_Normal_Distribution():
    ...
    assert_allclose(dist.mean, 5, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 2, rtol=tolerance)
    assert_allclose(dist.variance, 4, rtol=tolerance)
    assert_allclose(dist.skewness, 0, rtol=tolerance)
    assert_allclose(dist.kurtosis, 3, rtol=tolerance)
    assert dist.param_title_long == 'Normal Distribution (μ=5,σ=2)'
    assert_allclose(dist.quantile(0.2), 3.3167575328541714, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 3.9511989745839187, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(10), 0.6454895953278145, rtol=tolerance)
...
def test_Lognormal_Distribution():
    ...
    assert_allclose(dist.mean, 20.175674306073336, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 9.634600550542682, rtol=tolerance)
    assert_allclose(dist.variance, 92.82552776851736, rtol=tolerance)
    assert_allclose(dist.skewness, 3.689292296091298, rtol=tolerance)
    assert_allclose(dist.kurtosis, 34.36765343083244, rtol=tolerance)
    assert dist.param_title_long == 'Lognormal Distribution (μ=2,σ=0.8,γ=10)'
    assert_allclose(dist.quantile(0.2), 13.7685978648453116, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 14.857284757111664, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(20), 9.143537277214762, rtol=tolerance)
    xvals = [dist.gamma - 1, dist.quantile(0.001), dist.quantile(0.01), dist.quantile(0.1), dist.quantile(0.9), dist.quantile(0.99), dist.quantile(0.999)]
...
def test_Exponential_Distribution():
    ...
    assert_allclose(dist.mean, 15, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 5, rtol=tolerance)
    assert_allclose(dist.variance, 25, rtol=tolerance)
    assert_allclose(dist.skewness, 2, rtol=tolerance)
    assert_allclose(dist.kurtosis, 9, rtol=tolerance)
    assert dist.param_title_long == 'Exponential Distribution (λ=0.2,γ=10)'
    assert_allclose(dist.quantile(0.2), 11.11571775657105, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 11.783374719693661, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(20), 5, rtol=tolerance)
    xvals = [dist.gamma - 1, dist.quantile(0.001), dist.quantile(0.01), dist.quantile(0.1), dist.quantile(0.9), dist.quantile(0.99), dist.quantile(0.999)]
...
def test_Gamma_Distribution():
    ...
    assert_allclose(dist.mean, 20, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 7.0710678118654755, rtol=tolerance)
    assert_allclose(dist.variance, 50, rtol=tolerance)
    assert_allclose(dist.skewness, 1.414213562373095, rtol=tolerance)
    assert_allclose(dist.kurtosis, 6, rtol=tolerance)
    assert dist.param_title_long == 'Gamma Distribution (α=5,β=2,γ=10)'
    assert_allclose(dist.quantile(0.2), 14.121941545164923, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 15.486746053517457, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(20), 6.666666666666647, rtol=tolerance)
    xvals = [dist.gamma - 1, dist.quantile(0.001), dist.quantile(0.01), dist.quantile(0.1), dist.quantile(0.9), dist.quantile(0.99), dist.quantile(0.999)]
...
def test_Beta_Distribution():
    ...
    assert_allclose(dist.mean, 0.7142857142857143, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 0.15971914124998499, rtol=tolerance)
    assert_allclose(dist.variance, 0.025510204081632654, rtol=tolerance)
    assert_allclose(dist.skewness, -0.5962847939999439, rtol=tolerance)
    assert_allclose(dist.kurtosis, 2.88, rtol=tolerance)
    assert dist.param_title_long == 'Beta Distribution (α=5,β=2)'
    assert_allclose(dist.quantile(0.2), 0.577552475153728, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 0.6396423096199797, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(0.5), 0.2518796992481146, rtol=tolerance)
    xvals = [0, dist.quantile(0.001), dist.quantile(0.01), dist.quantile(0.1), dist.quantile(0.9), dist.quantile(0.99), dist.quantile(0.999)]
...
def test_Loglogistic_Distribution():
    ...
    assert_allclose(dist.mean, 61.308607648851535, rtol=tolerance)
    assert_allclose(dist.standard_deviation, 12.009521950735257, rtol=tolerance)
    assert_allclose(dist.variance, 144.228617485192, rtol=tolerance)
    assert_allclose(dist.skewness, 1.2246481827926854, rtol=tolerance)
    assert_allclose(dist.kurtosis, 8.342064360132765, rtol=tolerance)
    assert dist.param_title_long == 'Loglogistic Distribution (α=50,β=8,γ=10)'
    assert_allclose(dist.quantile(0.2), 52.044820762685724, rtol=tolerance)
    assert_allclose(dist.inverse_SF(q=0.7), 54.975179587474166, rtol=tolerance)
    assert_allclose(dist.mean_residual_life(20), 41.308716243335226, rtol=tolerance)
    xvals = [dist.gamma - 1, dist.quantile(0.001), dist.quantile(0.01), dist.quantile(0.1), dist.quantile(0.9), dist.quantile(0.99), dist.quantile(0.999)]
