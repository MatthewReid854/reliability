...
-    `Distributome <http://www.distributome.org/V3/calc/index.html>`_ - Provides PDF and CDF of a large number of prabability distributions that can be easily changed using sliders. It also includes a quantile / CDF calculator. Similar to the Distribution calculator below.
-    `Distribution Calculator <https://www.randomservices.org/random/apps/SpecialCalculator.html>`_ - this is a neat little app that allows you to quickly see what distributions look like using sliders for their parameters. A very large selection of distributions is available from the dropdown list. Parametrisations may differ from what you expect, but you'll soon figure out which one is which. It also includes a quantile / CDF calculator.
