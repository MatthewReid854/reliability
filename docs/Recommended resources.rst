...
-    R (Programming Language) - There are several useful libraries within R that can be used for reliability engineering. These include `WeibullR <https://github.com/openrelia/WeibullR.gallery>`_, `abrem <https://rdrr.io/rforge/abrem/>`_, and the `Survival analysis library <https://cran.r-project.org/web/packages/survival/index.html>`_. R is one of the most popular programming languages for survival analysis.
